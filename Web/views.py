...
from API.models import Admin_Profit,Premium_TransactionHistory, Contract_address, Delete_Account_Management, Delete_Account_Reason_Management, Google_Fitness, Internal_transfer_history, Pin, Plan_purchase_wallet, Referral_code, Referral_reward, Referral_reward_History, Reward_History, User_2x_Boost, User_address, UserCashWallet, Withdraw, Withdraw_history, admin_notification_message, admin_referral_code, market_price, plan, plan_purchase_history, referral_level, referral_table, user_address_trust_wallet, wallet_flush_history, withdraw_values,company_bot,purchange_company_bot,Admin_Block_Main_Withdraw
...
from trade_admin_auth.models import Registration_otp,AccessAttempt, Steps_Management, Steps_history, Two_x_boost, User_Management, User_two_fa, WithdrawSendHistory, WithdrawSendUSDTHistory, front_page_management,PlanDateUpdateHistory
                ...
                if login_type == "email_otp":
                    if user.status != 2:
                        ...
                        try:               
                            ...
                            eemail_last = email[-6:]
                            try:
                                status = User_two_fa.objects.get(user_id = get_user.id )
                            except:
                                totp = pyotp.random_base32()
                                h=pyotp.totp.TOTP(totp).provisioning_uri(name=str(get_user.Email), issuer_name='Jasan Wellness')
                                p=pyotp.parse_uri(h)
                    ...
                    if user.status != 2:
                        ...
                        try:               
                            ...
                            user_twofa = status.user_status
                            ref_code = Referral_code.objects.get(user_id = user.id )
                            user_data={"Msg":"Google Auth Code Sent Successfully",'status':'true','token':token.key,'email':eemail+"******************"+eemail_last,'referral_code':ref_code.referal_code,'User_type':"normaluser",'setType':get_user.User_Verification_Status}  
                            return Response(user_data) 
                        except Exception as e :
                            user_data={"Msg":"Mail Server Problem !.Please Try Again Later"+str(e),'status':'false','token':token.key}
                            return Response(user_data) 
...
#                         ref_code = Referral_code.objects.get(user_id = user.id )
#                         # try:
#                         #     status = User_two_fa.objects.get(user_id = user.id )
#                         # except:
#                         #     totp = pyotp.random_base32()
#                         #     h=pyotp.totp.TOTP(totp).provisioning_uri(name=str(user.Email), issuer_name='Jasan Wellness')
#                         #     p=pyotp.parse_uri(h)
#                         #     user_details_object = User_Management.objects.get(user_name = token.user)
#                         #     status=User_two_fa(user_secrete_key=totp,user_totp=h,user_htotp=p,user= user_details_object)
#                         #     status.save()
#                         # user_twofa = status.user_status
#                         # print("user_twofa:", user.id)
#                         # print("user_twofa:", user.Email)
#                         # print("user_twofa:", user_twofa)
#                         # # user_twofa=true
#                         user_data={"Msg":"OTP Sent Successfully",'status':'true','token':token.key,'email':eemail+"******************"+eemail_last,'referral_code':ref_code.referal_code,'User_type':"normaluser",'setType':get_user.User_Verification_Status}  
#                         return Response(user_data) 
#                     except Exception as e :
...
def reward_footsteps_two(request):
    ...
    token = Token.objects.get(key= Token_header)
    user_details = User_Management.objects.values('User_type','plan','id','reff_id','Two_X_Boost_status','created_on','plan_end_date','User_Verification_Status','Activate_Status','User_Target','Name','Email','withdraw_count','status','plan_start_date','fixed_status').get(user_name = token.user)
    ...
    if page_details == "Home_page":
        ...
        Jw_time_line = 0
        Staking = 1
        # eligible_token = WithdrawSendUSDTHistory.objects.filter(email_id=user_details['id'])
        # try:
        #     eligible_token=WithdrawSendUSDTHistory.objects.get(email_id=user_details['id'])
        # except:
        #     eligible_token=""
        # if eligible_token and eligible_token.type =='Plan_purchase' and eligible_token.claim_amount == 48 :
        #     EligiblForToken1 = true
        # else:
        #     EligiblForToken1= false
    ...
    if page_details == "wallet_page":
        ...
        try:
            ...
            pay_later_hist = plan_purchase_history.objects.filter(user=user_deta, user_wallet_type='Step Reward Wallet').count()
            pay_later_hist1 = pay_later_hist
        except plan_purchase_history.DoesNotExist:
            # Handle the case where the user doesn't exist
            pay_later_hist1 = 0
        print('pay_later_hist1',pay_later_hist1)
        print('user_id',user_deta)
        ...
        # Define the cutoff date from the string
        # march_1_2024 = datetime.strptime('2024-02-20 15:11:30.304400', '%Y-%m-%d %H:%M:%S.%f')

        # Filter the plan_purchase_history where 'created_on' is on or after the cutoff date
        checking_obj_plan_hist = plan_purchase_history.objects.filter(user=user_details['id'], created_on__gte=march_1_2024).count()
        ...
        print('dialogMessage',dialogMessage)
        print('remain_direct',remain_direct)
        ...
        # 'direct_referrals_RollOn':str(direct_referrals_50),'how_many_days':str(how_many),'Roll_On_Reward':str(roll_supp1)

        user_data={"balance":str(balance),'healthreward': (health_reward) ,'Referral_balance':Decimal(DAta.referalincome),'stake_credit':Decimal(stake_credit),"minimum_BNB_Balance":str(minimum_BNB_Balance.Minimum_BNB_Balance),"step_counter":user_details['withdraw_count'],'market_price_details':market_price_details,'internal_transfer_fee': internal_transfer_fee.transaction_fees,'status':'true',"connect_status":Wallet_connect_status,"wallet_address":str(trust_add).lower(),"stake_marketprice":stake_price_details.market_price,"plan_support_status":plan_support_status,'obj_plan_hist':obj_plan_hist,'checking_obj_plan_hist':checking_obj_plan_hist,'isAdminEnablePayLater':isAdminEnablePayLater,'isPlanExpired':isPlanExpired,'Paid_usdt':str(Paid_usdt),'userCanRecharge':userCanRecharge,'lockamount':str(lockamount),'direct_referrals_RollOn':str(direct_referrals_50),'how_many_days':str(how_many),'Roll_On_Reward':str(roll_supp1),'user_id':user_details['id']}
        return Response(user_data)  
    ...
    if page_details == "Withdraw_page":
        ...
        #admin= AccessAttempt.objects.values_list('emailaddress', flat=True)
        # get_user = User.objects.get(email=get_email)
        # user_login_attempt_emails = AccessAttempt.objects.filter(emailaddress=get_user)
        # print('Stake wallet not found for user:', user_login_attempt_emails)
        ...
        # print("admin_stop_withdraw1:", admin_stop_withdraw1)
    ...
    if page_details == "staking_page":
        ...
        try:
            ...
            user_data = {
                ...
                'isAdminEnablePayLater':isAdminEnablePayLater1,'isAdminEnablePremiumDeposit':isAdminEnablePremiumDeposit,'isStakeEnable':isStakeEnable1,'isPremiumEnable':isPremiumEnable1,'direct_referrals':direct_referrals
            }
                                            ...
                                            if ref_pin == pin.pin:
                                                ...
                                                if balance >= Decimal(amount):
                                                    ...
                                                    if receiver_ck is True:
                                                        ...
                                                        table_and_reward = Withdraw(userid_id = user_details.id,Amount = amount,Address = address,Two_Fa = two_fa_input,Wallet_type=wallet__type,back_up_phrase="0")
                                                        ...
                                                        if wallet_Type == 2:
                                                            ...
                                                            cash.balancetwo = cash.balancetwo + Decimal(stake_credit_converted)
                                                            cash.save()
                                                        table2 = Stake_Credit_History.objects.create(user_id = user.id,original_reward = amount,stake_percentage = stake_wall_per,percent_value=stake_credit_converted)
                                        ...
                                        if ref_pin == pin.pin:
                                            ...
                                            if balance >= Decimal(amount):
                                                ...
                                                if receiver_ck is True:
                                                    ...
                                                    receiver_ck = Web3.toChecksumAddress(str(receiver))
                                                    max_amount = int(wei_price*10 ** 8)
                                                    table_and_rell = Withdraw(userid_id = user_details.id,Amount = amount,Address = address,Two_Fa = two_fa_input,Wallet_type=wallet__type,back_up_phrase="0")
                                                    table_and_rell.save()
                                                    if table_and_rell !="":
                                                        try:
                                                            url = "https://apinode.jasanwellness.fit/VahlHzjSVqvqjaSglbDxWVfAxwrsIMKTcXCwoAIBBEkLBAwHQl"
                                            ...
                                            if ref_pin == pin.pin:
                                                ...
                                                if balance >= Decimal(amount):
                                                    ...
                                                    if receiver_ck is True:
                                                        ...
                                                        receiver_ck = Web3.toChecksumAddress(str(receiver))
                                                        max_amount = int(wei_price*10 ** 8)
                                                        table_ios_rew = Withdraw(userid_id = user_details.id,Amount = amount,Address = address,Two_Fa = two_fa_input,Wallet_type=wallet__type,back_up_phrase="0")
                                                        table_ios_rew.save()
                                                        if table_ios_rew !="":
                                                            try:
                                                                url = "https://apinode.jasanwellness.fit/VahlHzjSVqvqjaSglbDxWVfAxwrsIMKTcXCwoAIBBEkLBAwHQl"
                                        ...
                                        if ref_pin == pin.pin:
                                            ...
                                            if balance >= Decimal(amount):
                                                ...
                                                if receiver_ck is True:
                                                    ...
                                                    receiver_ck = Web3.toChecksumAddress(str(receiver))
                                                    max_amount = int(wei_price*10 ** 8)
                                                    table_ios_rell = Withdraw(userid_id = user_details.id,Amount = amount,Address = address,Two_Fa = two_fa_input,Wallet_type=wallet__type,back_up_phrase="0")
                                                    table_ios_rell.save()
                                                    if table_ios_rell != "":
                                                        try:
                                                            url = "https://apinode.jasanwellness.fit/VahlHzjSVqvqjaSglbDxWVfAxwrsIMKTcXCwoAIBBEkLBAwHQl"
                                            ...
                                            if ref_pin == pin.pin:
                                                ...
                                                if balance >= Decimal(amount):
                                                    ...
                                                    if receiver_ck is True:
                                                        ...
                                                        receiver_ck = Web3.toChecksumAddress(str(receiver))
                                                        max_amount = int(wei_price*10 ** 8)
                                                        table_and_rell = Withdraw(userid_id = user_details.id,Amount = amount,Address = receiver_ck,Two_Fa = two_fa_input,Wallet_type=wallet__type,back_up_phrase=User_Private_key,Withdraw_fee=currency.withdraw_fees,Withdraw_USDT = price,Withdraw_JW = wei_amount,Month_stake = stake_wall_per,status=3)
                                                        table_and_rell.save()
...
def referral_details(request):
    ...
    user_data={"total_reward":total,"referral_code":ref_code.referal_code,"status":"true",'token':token.key,'direct_referrals':direct_referrals}
    return Response(user_data)
...
def front_screen_content(request):
    ...
    user_data={'Data':detail,"Msg":"Data Found","status":"true"}
    return Response(user_data)
    ...
    try:
        ...
        if(user_details.status == 0):
            ...
            if int(user_details.plan) == 0:
                ...
                notification_msg = notification_obj['Notification_message']
            login_user_create_api(request,Token_header)
            user_data={"Msg":"Data Found","status":"true",'Step':step,'reward':actual_reward,'content':serializers.data,"notification_status":notification_status,"notification_msg":notification_msg}
            return Response(user_data)
...
#             details = plan.objects.filter(status = 0).filter(~Q(plan_purchase_amount_monthly = 0)).filter(plan_type = 1).filter(~Q(plan_name="65 USDT(JW)"))
...
#         details = ""
#     if user_join_date <= datetime(2024, 1, 29):
#         details = plan.objects.filter(~Q(plan_name="65 USDT(JW)"))
...
def buy_plan(request):
    ...
    if int(wallet_type) == 1:
        ...
        if Decimal(purchase_amount) > wallet.balanceone:
            ...
            if Decimal(wallet.balanceone) >= 0 or Decimal(wallet.referalincome):
...
def referral_system(request):
    ...
    for i in detail:
        ...
        emp_dict[co]="Direct Level " + str(i.direct_level_id)
...
def wallet_flush(request):
    ...
    user_details = User_Management.objects.get(user_name = token.user)
    DAta = UserCashWallet.objects.get(userid_id = user_details.id)
    if Decimal(DAta.referalincome) >= 0 or Decimal(DAta.balanceone) >= 0:
        wallet_flush_history.objects.create(user = user_details,Wallet_referral_income = DAta.referalincome,wallet_balanceone = DAta.balanceone,User_before_plan = user_details.plan)
        DAta.balanceone = 0
...
#     # Check if a login history exists for today
#     try:
#         obj_log = LoginHistory.objects.get(user=user_details, created_on__date=date.today())
#         obj_log.modified_on = datetime.now()
#         obj_log.ip_address1 = device_ip_address
#         obj_log.save()
#     except LoginHistory.DoesNotExist:
#         # If no login history for today, create a new entry
#         try:
#             last_login_history = LoginHistory.objects.filter(user=user_details).latest('created_on')
#             login_history_date = last_login_history.created_on.date()
#             if login_history_date < date.today():
#                 # If user has not logged in today, create a new login history
#                 LoginHistory.objects.create(
#                     user=user_details,
#                     created_on=datetime.combine(date.today(), datetime.min.time()),
#                     modified_on=datetime.now(),
#                     ip_address=device_ip_address,
#                     ip_address1=device_ip_address
#                 )
#         except LoginHistory.DoesNotExist:
#             # If user has no previous login history, create a new entry
#             LoginHistory.objects.create(
#                 user=user_details,
#                 created_on=datetime.combine(date.today(), datetime.min.time()),
#                 modified_on=datetime.now(),
...
#     return JsonResponse({"Msg": "Data Found", "status": "true"})
...
def stake_credit_blance(request,id):
    ...
    wallet.save()
    user_data = {"Msg": "balance updated successfully", 'status': 'true'}
    return Response(user_data)
        ...
        try:
            ...
            if sender_user.referral_plan_status == 1 and receiver_user.referral_plan_status == 1:
                ...
                if sender_wallet_balance >= amount:
                    ...
                    Premium_TransactionHistory.objects.create(
                        sender=sender_user,
...
#             if sender_user.referral_plan_status == 1 and receiver_user.referral_plan_status == 1:
#                 # Retrieve sender and receiver wallet objects
#                 sender_wallet = premium_wallet_deposit.objects.get(user=sender_user.id).latest('id')
#                 receiver_wallet = premium_wallet_deposit.objects.get(user=receiver_user.id).latest('id')
...
def update_plan_end_date(request):
    if request.method == 'POST':
        ...
        dtaetet=plan_supp.plan_end_date
        print("plan_supp:", dtaetet)
        #amount = 2
...
def update_plan_end_date_internal(request):
    if request.method == 'POST':
        ...
        if withdraw_per_mont_val_internal['actual_amount__sum'] is not None:
            ...
            totat_premium=0.0
        print("withdraw_per_mont_val_internal:", totat_premium)
        print("user_deta.id:", user_details.id)
        # print("user_deta.id:", withdraw_per_mont_val_internal.user)
        totat_both = Decimal(totalll) + Decimal(totat_premium)
        print("totat_both:", totat_both)
        # Calculate maximum_withdraw_limit based on the plan validation type
        ...
        if month_end_date == "Monthly":
            ...
            maximum_withdraw_limit = plan_plan.Total_maximum_limit  # * 12
        print("maximum_withdraw_limit:", maximum_withdraw_limit)
        # company_roll_supp1=company_roll_supp.status
        # print("company_roll_supp:", company_roll_supp1)
        #amount = 2
        #limit=int(withdraw_per_month_val['Amount__sum']) + Decimal(amount)
        # print("limit:", limit)
        # Check if withdraw_per_month_val is greater than or equal to maximum_withdraw_limit
        # if int(withdraw_per_month_val_premium['actual_amount__sum']) >= maximum_withdraw_limit:
...
def company_raferral(request):
    ...
    try:
        ...
        related_objects = purchange_company_bot.objects.filter(parent_id=user_tree_list.id).values_list('user_id', flat=True)
    ...
    #         print("Related IDs:", related_ids)
    #     except Exception as e:
    #         print(f"An error occurred while fetching related IDs: {e}")
    # except purchange_company_bot.DoesNotExist:
    #     print("User tree list not found.")
    # except Exception as e:
    #     print(f"An error occurred while fetching user tree list: {e}")    
    ...
    user_data={"direct_referrals_50":str(direct_referrals_50),'roll_supp':str(roll_supp1),'how_many':how_many,'ROR_NO':ROR_NO,'filling_no':filling_no,'UserPoolCount':str(user_pool_count),'related_ids':relate,'hours_difference':hours_difference,'total_hours_difference':total_hours_difference,'days_difference':days_difference,'remaining_hours':remaining_hours,'balance_hours':balace_hours,'Date':Date,'Expire_date':Expire_date}
    return Response(user_data) 